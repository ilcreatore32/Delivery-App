...
export const getShippments = async (req, res) => {
    ...
    switch (view_option) { 
        ...
        case 'carrier_taken':
          ...
          JOIN se_has_st
            ON SEST_SEId = SE_Id 
          JOIN (SELECT ST_Id FROM serviciotransporte WHERE ST_PersonaId = ${req.user.id}) ST
    ...
    try {
      ...
      await pool.query(queryShippment, function (error, results) {
        ...
        /* if there is no data */
        if (results.length === 0 || results[0].SE_Id === null) return res.status(404).json({error: "No posee solicitud de envío de productos"})
        /* send results */
...
export const getOneShippment = async function (req, res) {
  ...
  JOIN mediotransporte
	...
	WHERE SEST_SEId = ? AND SEST_Status <> 1 
  ...
  try {
    ...
    await pool.query(query_shippment, async function (errShippment, shippmentDetails) {
      /* if error in the query */
      if (errShippment) return res.status(400).json({error: "Error al consultar en la base de datos"})
      /* if there is no data */
      if (shippmentDetails.length === 0 || shippmentDetails[0].SE_Id === null) return res.status(404).json({error: "No posee solicitud de envío de productos"})
      
      /* Get all shippment's products */
      await pool.query(query_products, shippmentDetails[0].SE_Id, async function (errProduct, products) {
        /* if error in the query */
        if (errProduct) return res.status(400).json({error: "Error al consultar en la base de datos"})
        /* if there is no data */
        if (products.length === 0 || products[0].SE_Id === null) return res.status(404).json({error: "No posee solicitud de envío de productos"})
        ...
        if (shippmentDetails[0].SEST_STId !== null) {
          ...
          await pool.query(query_detail_service, shippmentDetails[0].SEST_STId, function (errServiceD, serviceD) {
            /* if error in the query */
            if (errServiceD) return res.status(400).json({error: "Error al consultar en la base de datos"})
            /* if there is no data */
            if (serviceD.length === 0 || serviceD[0].ST_Id === null) return res.status(404).json({error: "No posee solicitud de envío de productos"})
            /* send results */
            res.status(200).json( {shippmentDetails: shippmentDetails[0], productsList: products, serviceDetails: serviceD[0]}  )
          })
        } else {
          /* Return other available services */
          await pool.query(query_services, shippmentDetails[0].SE_Id, function (errServices, servicesAvailable){
            /* if error in the query */
            if (errServices) return res.status(400).json({error: "Error al consultar en la base de datos"})
            /* if there is no data  */
            if (servicesAvailable.length === 0 || servicesAvailable[0].ST_Id === null) return res.status(404).json({error: "No posee solicitud de envío de productos"})
            /* send results */
            res.status(200).json( {shippmentDetails: shippmentDetails[0], productsList: products, servicesAvailable: servicesAvailable}  )
          })
        } 
      })
    });
  ...
  try {
    ...
    await pool.query(query_shippment, async function (errShippment, shippmentDetails) {
      ...
      await pool.query(query_products, shippmentDetails[0].SE_Id, async function (errProduct, products) {
        ...
        if (products.length === 0 || products[0].SE_Id === null) return res.status(404).json({error: "No posee solicitud de envío de productos"})
        /* send results */
        res.status(200).json( {shippmentDetails: shippmentDetails[0], productsList: products}  )
      })
    });
  } catch (err) {
    /* error in the server */
    ...
    pool.getConnection(function(err, connection) {
      ...
      connection.beginTransaction(function(err) {
          ...
          if (err) {                  
              ...
              connection.query(queryUpdateDetails, function(err, updateDetailsResult) {
                  /* if error in the query */
                  if (err) {
                      /* Rollback the transaction */
                      connection.rollback(function() {
                    ...
                    if (SEST_Status && SEST_STId) {
                      connection.query(queryUpdateStatus, function(err, updateStatusResult) {
                        /* if error in the query */
                        if (err) {
                          /* Rollback the transaction */
                          connection.rollback(function() {
                          ...
                          /* Delete the products */
                          connection.query(queryDeleteProducts, function(err, deleteProductsResult) {
                            /* if error in the query */
                            if (err) {
                              /* Rollback the transaction */
                              connection.rollback(function() {
                              ...
                              /* Insert the products */
                              connection.query(queryUpdateProducts,[productsList], function(err, updateProductsResult) {
                                /* if error in the query */
                                if (err) {
                                  /* Rollback the transaction */
                                  connection.rollback(function() {
